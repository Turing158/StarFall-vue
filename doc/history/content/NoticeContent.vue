            ...
            <table class="noticeBlock">
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <div class="sendBlock">
                            <textarea type="text" class="sendBox" v-model="contentValue" :disabled="disableSend"></textarea>
                            <div class="operate">
                                <div>
                                    <McBtn text="发送" @click="send()" :disabled="disableSend"/>
                                </div>
                            </div>
                        </div>
                    </td>
...
import NoticeUserView from '@/components/ChatCloudItem.vue';
import McBtn from '@/components/McBtn.vue';
import { findMessageList, findMsgByToUserAndFromUser, sendMessage } from '@/api/friend';
...
const init = async()=>{
    ...
    disableSend.value = noticeUserList.value[listIndex.value].name == 'StarFall官方' ? true : false
...
const listClick = (item,index)=>{
    ...
    disableSend.value = item.name == 'StarFall官方' ? true : false
...
const send = async()=>{
    ...
    await sendMessage(noticeUserList.value[indexTmp].user,contentValue.value).then(res=>{
        ...
        else if(msg == 'DISABLE_TO_STARFALL'){
            ElMessage.error('不允许发送消息至官号');
        }
        else{
